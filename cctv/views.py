...
def space_manage(request):
    ...
    with connection.cursor() as form:
        ...
        if request.method == "POST" and request.POST['mode'] =="insert" :
            ...
            form.execute("DELETE FROM cctv_sequence WHERE id IN ( SELECT sequence_id FROM cctv_sequence_connects WHERE neighborhood_id IN ( SELECT id FROM cctv_neighborhood WHERE space_a_id = %s or space_b_id = %s))",[request.POST['shoot_space_id'],request.POST['shoot_space_id']] )
            form.execute("DELETE FROM cctv_sequence_connects WHERE sequence_id IN ( SELECT sequence_id FROM cctv_sequence_connects WHERE neighborhood_id IN ( SELECT id FROM cctv_neighborhood WHERE space_a_id = %s or space_b_id = %s))",[request.POST['shoot_space_id'],request.POST['shoot_space_id']] )
            form.execute("DELETE FROM cctv_neighborhood WHERE space_a_id = %s or space_b_id = %s", [request.POST['shoot_space_id'],request.POST['shoot_space_id']])
        ...
        #neighborhood
            ...
            form.execute("DELETE FROM cctv_sequence WHERE id IN ( SELECT sequence_id FROM cctv_sequence_connects WHERE neighborhood_id = %s)",[request.POST['neighbor_id']])
            form.execute("DELETE FROM cctv_sequence_connects WHERE sequence_id IN ( SELECT sequence_id FROM cctv_sequence_connects WHERE neighborhood_id = %s)", [request.POST['neighbor_id']])
            ...
            form.execute("UPDATE cctv_neighborhood SET route = %s, location = %s, space_a_id = %s, space_b_id = %s WHERE id = %s", [request.POST['route'], request.POST['loc'], request.POST['space_a_id'],request.POST['space_b_id'], request.POST['neighbor_space_id']])
        #sequence
        elif request.method == "POST" and request.POST['mode'] == "insertSequence":
            form.execute("INSERT INTO cctv_sequence ('id', 'last_id') VALUES( %s, %s )", [request.POST['seq_id'], request.POST['neighbor_id']])
            form.execute("INSERT INTO cctv_sequence_connects ('sequence_id', 'neighborhood_id') VALUES( %s, %s )", [request.POST['seq_id'], request.POST['neighbor_id']])
            ...
            if temp_b==temp_a:
                form.execute("INSERT INTO cctv_sequence_connects ('sequence_id', 'neighborhood_id') VALUES( %s, %s )",[request.POST['seq_id'], request.POST['neighbor_id']])
                form.execute("UPDATE cctv_sequence SET last_id = %s WHERE id = %s", [request.POST['neighbor_id'], request.POST['seq_id']])
    ...
    return render(request, 'cctv/space_manage.html', {'space_list' : space_list, 'neighbor_list' : neighbor_list,'sequences' : sequences, 'sequence_list' : sequence_list})
